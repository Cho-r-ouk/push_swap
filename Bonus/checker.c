...
/*   Updated: 2024/01/30 10:36:41 by cmasnaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move(int *tab, char *buff, int *size_a, int *size_b)
{
...
int	apply_moves(int *tab, int size_a, int *size_b)
{
	int		n;
	int		b;
	char	*buff;
...
int	main(int ac, char **av)
{
	...
	if (read(0, "", 0) < 0)
		return (free(tab), write (2, "Error\n", 6), 1);
	if (apply_moves(tab, tab_len, &size_b) != 1)
		return (free(tab), 1);
